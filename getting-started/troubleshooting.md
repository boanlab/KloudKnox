...
1. **The test invokes the binary directly through `kubectl exec`** (e.g. `kubectl exec <pod> -- /bin/sleep`). On the AppArmor fallback path this does not trigger `deny <path> x,` — see [enforcement-with-apparmor.md](enforcement-with-apparmor.md). Wrap the test in a shell:
